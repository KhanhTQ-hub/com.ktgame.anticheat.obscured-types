namespace com.ktgame.anticheat.obscured_types
{
	...
	public partial struct ObscuredVector3 : IObscuredType
	{
		...
		[SerializeField] internal Vector3 fakeValue;
		...
		public float x
		{
			get
			{
				...
				// 					                 $"epsilon: {ObscuredCheatingDetector.Instance.vector3Epsilon}" +
		...
		public float z
		{
			get
			{
				...
				// 				if (ObscuredCheatingDetector.ExistsAndIsRunning && fakeValueActive && !Compare(decrypted, fakeValue.z))
				// 				{
				// #if ACTK_DETECTION_BACKLOGS
				// 					Debug.LogWarning(ObscuredCheatingDetector.LogPrefix + "Detection backlog:\n" +
				// 					                 $"type: {nameof(ObscuredVector3)}\n" +
				...
				// 					                 $"epsilon: {ObscuredCheatingDetector.Instance.vector3Epsilon}" +
				// 					                 $"compare: {Compare(decrypted, fakeValue.z)}");
				// #endif
				// 					ObscuredCheatingDetector.Instance.OnCheatingDetected(this, decrypted, fakeValue);
				// 				}
		...
		{
			var epsilon = /*ObscuredCheatingDetector.ExistsAndIsRunning ? ObscuredCheatingDetector.Instance.vector3Epsilon :*/ float.Epsilon;
